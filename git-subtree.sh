...
# Normalize the prefix to remove any trailing '/'.
dir="$(echo "$prefix/" | sed s,/*$,,)"
...
ensure_clean()
{
	if ! git diff-index HEAD --exit-code --quiet > /dev/null 2>&1; then
		die "Working tree has modifications.  Cannot add."
	fi
	if ! git diff-index --cached HEAD --exit-code --quiet > /dev/null 2>&1; then
...
cmd_add()
{
	...
	if [ $# -eq 1 ]; then
	    ...
	    die "You must provide either exactly one refspec or else a repository and refspec: Got '$revs'"
	fi
}
